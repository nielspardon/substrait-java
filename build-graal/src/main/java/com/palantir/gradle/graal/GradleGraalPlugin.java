 ...
 *
 * <p>All tooling execution (e.g. nativeImage) will cause GraalVM tooling to download and cache if
 * not already present. Currently, GraalVM CE only supports MacOS and Linux, and, as a result, this
 * plugin will only correctly function on MacOS and Linux. The plugin will automatically select the
 * correct architecture and error clearly if the runtime architecture is not supported.
 *
...
public class GradleGraalPlugin implements Plugin<Project> {
  ...
  public final void apply(final Project project) {
    ...
    final GraalExtension extension =
        project.getExtensions().create("graal", GraalExtension.class, project);

    final Path cacheDir =
        Optional.ofNullable((String) project.findProperty("com.palantir.graal.cache.dir"))
            .map(Paths::get)
            .orElseGet(
                () ->
                    project
    ...
    final TaskProvider<DownloadGraalTask> downloadGraal =
        project
            .getTasks()
            .register(
    ...
    final TaskProvider<ExtractGraalTask> extractGraal =
        project
            .getTasks()
            .register(
                "extractGraalTooling",
    ...
    final TaskProvider<Jar> jar = project.getTasks().withType(Jar.class).named("jar");
    project
        .getTasks()
        .register(
            "nativeImage",
    ...
    final TaskProvider<Jar> sharedLibrary = project.getTasks().withType(Jar.class).named("jar");
    project
        .getTasks()
        .register(
            "sharedLibrary",
