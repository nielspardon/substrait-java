...
public class ExtractGraalTask extends DefaultTask {
  ...
  public ExtractGraalTask() {
    ...
    setDescription(
        "Extracts GraalVM tooling from downloaded archive using the system's tar command or Gradle's"
            + " copy method.");
  ...
  public final void extractGraal() {
    ...
    final File inputArchiveFile = inputArchive.get().getAsFile();
    final Path versionedCacheDir =
        cacheDir.get().resolve(Paths.get(graalVersion.get(), javaVersion.get()));
    ...
    if (inputArchiveFile.getName().endsWith(".zip")) {
      ...
      // ideally this would be a CopyTask, but through Gradle 4.9 CopyTask fails to correctly
      // extract symlinks
      project
          .getProviders()
          .exec(
    ...
    final File nativeImageExecutable = getExecutable("native-image");
    if (!nativeImageExecutable.isFile()) {
      project
          .getProviders()
          .exec(
              spec -> {
                final File graalUpdateExecutable = getExecutable("gu");
  ...
  private File getExecutable(final String binaryName) {
  ...
  private Path getArchitectureSpecifiedBinaryPath(final String binaryName) {
  ...
  public final void setInputArchive(final Provider<RegularFile> value) {
