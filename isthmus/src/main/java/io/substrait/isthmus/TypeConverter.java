...
public class TypeConverter {
  ...
  public NamedStruct toNamedStruct(final RelDataType type) {
    ...
    final var struct = (Type.Struct) toSubstrait(type, names);
  ...
  private Type toSubstrait(final RelDataType type, final List<String> names) {
    ...
    final var userType = userTypeMapper.toSubstrait(type);
    if (userType != null) {
      return userType;
    }
    ...
    return switch (type.getSqlTypeName()) {
      ...
      case ROW -> {
        final var children = new ArrayList<Type>();
  ...
  private static class ToRelDataType
    ...
    public RelDataType visit(final Type.UserDefined expr) throws RuntimeException {
      final var type = userTypeMapper.toCalcite(expr);
      if (type != null) {
        return type;
      }
      throw new UnsupportedOperationException(
